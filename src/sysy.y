...
%token COMMA SEMICOLON LEFT_BRACKET T LEFT_PAREN RIGHT_PAREN LEFT_CURLY_BRACKET RIGHT_CURLY_BRACKET
...
Stmt : LVal ASSIGN Exp SEMICOLON                                 {$$=new StmtNode();$$->addSon($1);$$->addSon(new AssignNode());$$->addSon($3);$$->addSon(new SemicolonNode());}
     ...
     | IF LEFT_PAREN Exp RIGHT_PAREN Stmt %prec LOWER_THAN_ELSE  {$$=new StmtNode();$$->addSon(new IfNode());$$->addSon(new LeftParenNode());$$->addSon($3);$$->addSon(new RightParenNode());$$->addSon($5);}
